...
| a Mate's bot user | `mate-{projectId}` (login), full name the Mate's name; restricted, `max_repo_creation 0`, no org creation; retired (tokens deleted, login prohibited) once its project is deleted in Zerops, allowed to sign in again once the project is listed again |
| a bot's token | `mate/{bot}/{generation}`, generation from 1; scopes `write:repository,read:user`; never the newest nor the one the Mate's container holds (the held one); an older generation revoked once the held one is past the grace, a newer one (or any, when the container holds none) once its successor is; a missing creation time blocks and is reported; one bot's cleanup counts once against its group's cap |
...
| protected branches | `main` on every repository: no direct push for anyone, merge by anyone with write (the `write` and `release` teams; on the group repo the broker merges a Mate's recipe pull request itself when it only adds files, D23 — one that changes a file `main` carries waits for a person); `env/*` on the group repo: the broker only; tags `v*` on the group repo: the `release` team. `main` is born with an initial commit, so a Mate's branch must descend from it (zcp fetches `main` before it branches) — an unrelated history merges only by rebase (measured 2026-09-16). The API merge is refused to the site admin when a merge whitelist is set; only a member of the whitelisted team merges |
